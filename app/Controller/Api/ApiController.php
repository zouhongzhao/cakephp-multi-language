...
// 		}
	...
	function vlogin($url, $data) {
		...
		curl_setopt($this->_curl, CURLOPT_COOKIE, session_name() . '=' . session_id());
// 		@curl_setopt ( $this->_curl, CURLOPT_FOLLOWLOCATION, 1 );
		curl_setopt ( $this->_curl, CURLOPT_AUTOREFERER, 1 );
		curl_setopt ( $this->_curl, CURLOPT_POST, 1 ); 
		curl_setopt ( $this->_curl, CURLOPT_POSTFIELDS, http_build_query($data) );
		curl_setopt ( $this->_curl, CURLOPT_COOKIEJAR, $this->_cookie_file ); 
		curl_setopt ( $this->_curl, CURLOPT_COOKIEFILE, $this->_cookie_file ); 
...
// 		curl_setopt($curl, CURLOPT_FOLLOWLOCATION, true);
		...
		$tmpInfo = curl_exec ( $this->_curl ); 
		if (curl_errno ( $this->_curl )) {
			return 'Errno' . curl_error ( $this->_curl );
		}
		return $tmpInfo; 
	}
...
// 		$curl = $this->_curl;
		curl_setopt ( $this->_curl, CURLOPT_URL, $url ); 
		curl_setopt($this->_curl, CURLOPT_HTTPHEADER, $this->_headers2);
		curl_setopt ( $this->_curl, CURLOPT_SSL_VERIFYPEER, 0 ); 
		curl_setopt ( $this->_curl, CURLOPT_SSL_VERIFYHOST, 1 ); 
		curl_setopt ( $this->_curl, CURLOPT_USERAGENT, $this->_user_agent );
// 		@curl_setopt ( $this->_curl, CURLOPT_FOLLOWLOCATION, 1 ); 
...
// 		curl_close ( $curl ); // 关闭CURL会话
	...
	function delcookie($_cookie_file) { 
		unlink ( $_cookie_file );
	}
